...
export const Games: React.FC = () => {
  ...
  const [showAddModal, setShowAddModal] = useState(false);
  const [loading, setLoading] = useState(false);
  const [newGame, setNewGame] = useState<{ name: string; imageFile?: File | null; link: string }>({
    name: "",
    imageFile: null,
    link: "",
  });
  ...
  const fetchGames = async () => {
    setLoading(true);
    try {
      const data = useMock ? mockGames : await GameService.list();
      setGames(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const created = useMock
        ? {
            id: `game_${Date.now()}`,
            imageUrl: newGame.imageFile ? URL.createObjectURL(newGame.imageFile) : "/placeholder.svg",
            ...newGame,
          }
        ...
        <div className="admin-card">
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <a
                          ...
                          className="inline-flex items-center gap-1 text-primary hover:underline"
                        >
                          <LinkIcon className="h-4 w-4" /> {new URL(item.link).hostname}
                        </a>
                      </td>
                      <td>
                        <div className="flex items-center gap-2">
          ...
          {totalPages > 1 && (
            <div className="p-6 border-t border-border">
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Showing {startIndex + 1} to {endIndex} of {games.length} results
                </p>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => setNewGame({ ...newGame, imageFile: e.target.files?.[0] || null })}
                  className="w-full p-2 border border-border rounded-lg"
                />
              </div>
