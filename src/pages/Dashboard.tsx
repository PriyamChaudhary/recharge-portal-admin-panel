...
import { ChartContainer, ChartTooltip, ChartTooltipContent } from "@/components/ui/chart";
import { dashboardService, DashboardStats, DashboardCharts } from "@/services/dashboardService";
import { mockDashboardStats, mockChartData } from "@/mocks/data";

const StatCard = ({ title, value, growth, icon: Icon, trend = "up", onClick }: { 
  title: string; value: string | number; growth: number; icon: React.ComponentType<any>; trend?: "up" | "down"; onClick?: () => void; 
}) => (
  <div 
    className="admin-card p-6 cursor-pointer hover:shadow-lg transition-shadow duration-200 hover:scale-[1.02]"
    onClick={onClick}
...
const QuickAction = ({ title, icon: Icon, onClick }: { title: string; icon: React.ComponentType<any>; onClick: () => void }) => (
  <button
    onClick={onClick}
    className="admin-card p-4 hover:shadow-lg transition-shadow text-left group"
  >
