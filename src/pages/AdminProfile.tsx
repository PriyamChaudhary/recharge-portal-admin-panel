...
import { useAuth } from "@/contexts/AuthContext";
import { PinInput } from "@/components/ui/pin-input";
import { adminService, ProfileData, PasswordData } from "@/services/adminService";
...
export const AdminProfile = () => {
  ...
  // Fetch profile, activity, and permissions from backend
  useEffect(() => {
  ...
  const handleChangeMpin = async () => {
    ...
    try {
      await adminService.changeMpin({ mpin: newMpin });
      setShowChangeMpin(false);
      setNewMpin("");
      setConfirmMpin("");
    } catch (err) {
