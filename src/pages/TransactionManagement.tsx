...
import { getTransactions, Transaction, updateTransactionStatus } from '@/services/transactionService';
...
export const TransactionManagement = () => {
  ...
  const updateStatus = async (transactionId: string, newStatus: Transaction['status']) => {
    setTransactions(prev =>
  ...
  const initiateRefund = (transactionId: string) => updateStatus(transactionId, 'refunded');
  ...
  const filteredTransactions = transactions
    .filter(t => {
      ...
      const matchesService = serviceFilter === 'all' || t.type === serviceFilter;
      ...
      if (amountRangeFilter !== 'all') {
        if (amountRangeFilter === 'low') matchesAmount = t.amount < 100;
        else if (amountRangeFilter === 'medium') matchesAmount = t.amount >= 100 && t.amount < 1000;
        else if (amountRangeFilter === 'high') matchesAmount = t.amount >= 1000;
      }

      return matchesSearch && matchesStatus && matchesService && matchesDate && matchesAmount;
    })
    .sort((a, b) => {
      let comparison = 0;
      switch (sortBy) {
        ...
        case 'user': comparison = a.userName.localeCompare(b.userName); break;
        case 'service': comparison = a.service.localeCompare(b.service); break;
      }
      return sortOrder === 'desc' ? -comparison : comparison;
    });

  const exportTransactions = () => console.log('Exporting transactions...', filteredTransactions);

  return (
    <AdminLayout title="Transaction Management">
      <div className="p-6">
        {/* Header with Filters */}
        {/* ... same header/filter UI code ... */}
        {/* Transactions Table */}
        {loading ? (
          <div className="p-8 text-center text-muted-foreground">Loading...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="admin-table">
              <thead>
                <tr>
                  <th>ID</th><th>User</th><th>Service</th><th>Amount</th><th>Commission</th><th>Status</th><th>Date</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredTransactions.map(t => (
                  <tr key={t.id}>
                    <td>{t.id}</td>
                    <td>{t.userName} <div className="text-sm text-muted-foreground">{t.userId}</div></td>
                    <td>{t.service} <div className="text-sm text-muted-foreground">{t.type}</div></td>
                    ...
                    <td>
                      {t.status === 'pending' ? (
                        <select
                          value={t.status}
                          onChange={(e) => updateStatus(t.id, e.target.value as Transaction['status'])}
                          className="text-sm border border-border rounded px-2 py-1"
                        >
                          <option value="pending">Pending</option>
                          <option value="success">Success</option>
                        ...
                        <span className={`${getStatusBadge(t.status)} capitalize`}>{t.status}</span>
                      )}
                    </td>
                    <td>{new Date(t.createdAt).toLocaleDateString()}</td>
                    <td>
                      {(t.status === 'success' || t.status === 'failed') && (
                        <button onClick={() => initiateRefund(t.id)} title="Refund">
                          <ArrowPathIcon className="h-4 w-4" />
                        </button>
                      )}
                    </td>
