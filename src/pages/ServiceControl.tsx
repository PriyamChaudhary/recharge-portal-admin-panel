...
import React, { useEffect, useState } from 'react';
import { AdminLayout } from '@/components/layout/AdminLayout';
import { CogIcon, PlusIcon, XMarkIcon, KeyIcon, EyeIcon, EyeSlashIcon } from '@heroicons/react/24/outline';
...
export const ServiceControl = () => {
  ...
  formData.append(key, String(value));
    ...
    try {
      const created = await ServiceService.create(formData);
  ...
  // Update Service
  const handleUpdateService = async (id: string, updatedFields: Record<string, any>) => {
    const formData = new FormData();
    ...
    try {
      const updated = await ServiceService.update(id, formData);
      ...
      <div className="p-6">
        <div className="admin-card">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((service) => (
              <div key={service.id} className="border border-border p-6 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h3 className="font-semibold">{service.name}</h3>
                    <p className="text-sm text-muted-foreground">{service.description}</p>
                  </div>
                  <button onClick={() => handleDeleteService(service.id)} className="btn-secondary text-sm">
                    Delete
                  </button>
                </div>
                <div className="text-sm">
                  <p>Provider: {service.provider}</p>
                  <p>Section: {service.section}</p>
                  <p>Status: {service.active ? 'Active' : 'Inactive'}</p>
                  <p>API Key: {service.apiKey ? service.apiKey : 'Not Configured'}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-background rounded-lg p-6 w-full max-w-md">
              ...
              <input
                ...
                onChange={(e) => setNewService({ ...newService, name: e.target.value })}
                className="w-full p-3 border border-border rounded mb-2"
              />
              <input
                type="text"
                placeholder="API Key"
