...
import { AdminLayout } from '@/components/layout/AdminLayout';
import { 
  CheckCircleIcon, XCircleIcon, EyeIcon, ChatBubbleLeftRightIcon 
} from '@heroicons/react/24/outline';
import { KYCRequest, KYCStatus, getKYCList, manageKYC } from '@/services/kycService';
...
export const KYCManagement = () => {
  ...
  const handleApprove = async (requestId: string) => {
    try {
      const updated = await manageKYC(requestId, 'verified', 'KYC approved successfully');
  ...
  const submitRejection = async () => {
    ...
    try {
      const updated = await manageKYC(selectedRequest, 'rejected', comments.trim());
      setRequests(prev => prev.map(r => r.id === selectedRequest ? updated : r));
      ...
      <div className="p-6">
        <div className="admin-card">
          ...
          <div className="border-b border-border">
            <nav className="flex space-x-8 px-6">
              {tabs.map(tab => (
                ...
                >
                  {tab.label} <span className="ml-2 bg-accent text-accent-foreground rounded-full px-2 py-1 text-xs">{tab.count}</span>
                </button>
              ))}
            </nav>
          </div>
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">No {activeTab} KYC requests found.</div>
            ) : (
              <div className="space-y-4">
                {filteredRequests.map(r => (
                  <div key={r.id} className="border border-border rounded-lg p-4 flex flex-col lg:flex-row lg:items-center gap-4">
                    <img src={r.documentUrl} alt="Document" className="w-24 h-16 object-cover rounded border border-border flex-shrink-0"/>
                    <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="font-semibold text-foreground">{r.userName}</h4>
                        <p className="text-sm text-muted-foreground">ID: {r.userId}</p>
                        <p className="text-sm text-muted-foreground mt-1">{r.documentType}: {r.documentNumber}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Submitted: {new Date(r.submittedAt).toLocaleDateString()}</p>
                        ...
                        {r.comments && <p className="text-sm text-muted-foreground mt-2"><strong>Comments:</strong> {r.comments}</p>}
                      </div>
                    </div>
                    {r.status === 'pending' && (
                      <div className="flex flex-row lg:flex-col gap-2">
                        <button onClick={() => handleApprove(r.id)} className="bg-success hover:bg-success/90 text-success-foreground px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
                          <CheckCircleIcon className="h-4 w-4" /> Approve
                        </button>
                        <button onClick={() => handleReject(r.id)} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
        ...
        {showCommentsModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-card p-6 rounded-xl max-w-md w-full mx-4">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <ChatBubbleLeftRightIcon className="h-5 w-5" /> Rejection Comments
              </h3>
              <textarea
                value={comments}
                onChange={e => setComments(e.target.value)}
              ...
              />
              <div className="flex justify-end gap-3 mt-4">
                <button onClick={() => setShowCommentsModal(false)} className="btn-secondary">Cancel</button>
                <button onClick={submitRejection} disabled={!comments.trim()} className="btn-primary disabled:opacity-50 disabled:cursor-not-allowed">Submit Rejection</button>
              </div>
            </div>
          </div>
        )}
