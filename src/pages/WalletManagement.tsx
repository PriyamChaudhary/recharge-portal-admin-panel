...
export const WalletManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [reqs, usrs] = await Promise.all([getWalletRequests(), getUsers()]);
        setRequests(reqs);
        setUsers(usrs);
      } catch (err) {
  ...
  const handleApprove = async (requestId: string) => {
    ...
    await updateWalletRequestStatus(requestId, 'ACCEPT');
  };

  const handleReject = async (requestId: string) => {
    setRequests(prev => prev.map(r => r.id === requestId ? { ...r, status: 'rejected' } : r));
    await updateWalletRequestStatus(requestId, 'DECLINE');
  };
  ...
  const handleSubmit = async () => {
    if (selectedUser && amount && reason && modalType) {
      await manualWalletUpdate(selectedUser, modalType, Number(amount), reason);
      closeModal();
    }
  };
  ...
  if (loading) return <AdminLayout title="Wallet Management"><div className="p-6">Loading...</div></AdminLayout>;
