...
import { 
  ...
  ChevronLeftIcon, ChevronRightIcon, ClipboardDocumentIcon
} from '@heroicons/react/24/outline';
import { saveAs } from 'file-saver';
import Papa from 'papaparse';
import { Switch } from '@/components/ui/switch';
...
export const UserManagement = () => {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      setLoading(true);
      try {
        if (USE_MOCK) setUsers(mockUsers);
        else {
  ...
  const filteredUsers = users.filter(user => {
    ...
    if (searchTerm) {
      switch (searchFilter) {
        case 'name': matchesSearch = user.name.toLowerCase().includes(searchTerm.toLowerCase()); break;
        case 'id': matchesSearch = user.id.toLowerCase().includes(searchTerm.toLowerCase()); break;
        case 'email': matchesSearch = user.email.toLowerCase().includes(searchTerm.toLowerCase()); break;
        case 'phone': matchesSearch = user.mobile.includes(searchTerm); break;
      }
    }
    let matchesStatus = true;
    if (sortFilter === 'active') matchesStatus = user.status === 'active';
    if (sortFilter === 'inactive') matchesStatus = user.status !== 'active';
    return matchesSearch && matchesStatus;
  }).sort((a, b) => {
    if (sortFilter === 'oldest') return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
    if (sortFilter === 'latest') return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    return 0;
  });
  ...
  const saveReferral = async () => {
    if (!selectedUserForReferral || !referralInput.trim()) return;
    try {
      await userService.addReferral(selectedUserForReferral, referralInput.trim());
      setUsers(users.map(u => u.id === selectedUserForReferral ? { ...u, referredBy: referralInput.trim() } : u));
  ...
  const exportToCSV = () => {
    const csv = Papa.unparse(filteredUsers);
    saveAs(new Blob([csv], { type: 'text/csv;charset=utf-8;' }), `users_${new Date().toISOString().split('T')[0]}.csv`);
  };
  ...
  if (loading) return <AdminLayout title="User Management"><div className="p-6">Loading users...</div></AdminLayout>;
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between mb-4">
          <div className="flex gap-2">
            ...
            <div className="relative">
              <MagnifyingGlassIcon className="h-5 w-5 absolute left-3 top-1/2 -translate-y-1/2" />
              <Input className="pl-10 w-64" placeholder={`Search by ${searchFilter}...`} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
            <Select value={sortFilter} onValueChange={setSortFilter}>
              <SelectTrigger className="w-40"><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <div className="overflow-x-auto">
          <table className="admin-table">
            ...
            <tbody>
              {currentUsers.map(user => (
                <tr key={user.id}>
                  <td><button onClick={() => copyToClipboard(user.id, 'User ID')}><ClipboardDocumentIcon className="h-5 w-5" /></button></td>
                  <td>{user.name}</td>
                  <td>{user.mobile}</td>
                  <td>{user.email}</td>
                  <td>{user.referredBy || <Button size="sm" variant="outline" onClick={() => handleAddReferral(user.id)}>Add</Button>}</td>
                  <td>{formatDate(user.createdAt)}</td>
                  <td>{user.ipAddress}</td>
                  <td>{user.address}</td>
                  <td><Switch checked={user.status === 'active'} onCheckedChange={c => handleStatusChange(user.id, c ? 'active' : 'suspended')} /></td>
                  <td>{visibleMpin[user.id] ? user.mpin : '****'} <Button size="sm" variant="ghost" onClick={() => toggleMpinVisibility(user.id)}><EyeIcon className="h-3 w-3" /></Button></td>
                  <td>₹{user.walletBalance.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
        ...
        {totalPages > 1 && (
          <div className="flex justify-between mt-4">
            <p>Showing {(currentPage-1)*itemsPerPage+1}-{Math.min(currentPage*itemsPerPage, filteredUsers.length)} of {filteredUsers.length}</p>
            <div className="flex gap-2">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage-1)}><ChevronLeftIcon className="h-4 w-4" /></button>
              <span>{currentPage} / {totalPages}</span>
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage+1)}><ChevronRightIcon className="h-4 w-4" /></button>
            </div>
          </div>
        )}
        ...
        <Dialog open={referralModalOpen} onOpenChange={setReferralModalOpen}>
          <DialogContent>
            ...
            <Input placeholder="Enter referrer ID/email" value={referralInput} onChange={e => setReferralInput(e.target.value)} />
            <div className="flex justify-end gap-2 mt-4">
              <Button variant="outline" onClick={() => setReferralModalOpen(false)}>Cancel</Button>
              <Button onClick={saveReferral}>Save</Button>
            </div>
